...
const NumberControl = ({ number, onClick, completionPercentage }) => (
  <div
    ...
    className="number"
    onClick={onClick}
    style = {{backgroundColor: numToColor(number)}}
  >
    {/* <div style = {{backgroundColor: numToColor(number)}}></div> */}
    ...
    <div className={`cell`} onClick={onClick} style = {{backgroundImage: `url("wood.jpg")`}}>
      {
        ...
        //   range(9).map(i =>
        //     (
        //       <div key={i} className="note-number" style = {{backgroundColor: numToColor(i+1)}}>
        //         {notes.has(i + 1) && numToColor(i + 1)}
        //         {/* //{numToColor(1)} */}
        //       </div>
        //     )) :
